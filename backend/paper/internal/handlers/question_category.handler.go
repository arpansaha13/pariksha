...
func (s *PaperServer) UpdateCategory(ctx context.Context, req *proto.UpdateCategoryRequest) (*proto.Empty, error) {
	category, ok := ctx.Value(interceptors.CategoryCtxKey{}).(models.QuestionCategory)
	if !ok {
...
func (s *PaperServer) DeleteCategory(ctx context.Context, req *proto.CategoryRequest) (*proto.Empty, error) {
	...
	err := db.DB.Transaction(func(tx *gorm.DB) error {
		...
		var questionCounts json.RawMessage = paper.QuestionCounts
		var err error

		// Update counts for each question type
		for _, q := range questions {
			questionCounts, err = updateQuestionCounts(questionCounts, q.Type, -1)
			if err != nil {
				return err
			}
