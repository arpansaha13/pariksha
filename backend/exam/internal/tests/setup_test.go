...
func TestMain(m *testing.M) {
	mockServer, mockDbInst, mockQuestionIntSvc, mockIntc, cleanup := modules.NewMock()
	defer cleanup()
...
func setupGrpcServer(server proto.ExamServer, mockIntc []grpc.UnaryServerInterceptor) (*grpc.Server, *grpc.ClientConn) {
	const bufSize = 1024 * 1024
	lis := bufconn.Listen(bufSize)
	srv := grpc.NewServer(grpc.ChainUnaryInterceptor(mockIntc...))
	proto.RegisterExamServer(srv, server)
